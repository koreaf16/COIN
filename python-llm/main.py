...
from llm import generate, parse_json_response, generate_deepseek_plan
...
from prompts import (
    ...
    build_unified_plan_context,
    build_unified_plan_data,
...
async def unified_plan(req: UnifiedPlanRequest):
    ...
    if route == "cloud":
        ...
        # 4-message 구조: 느린 컨텍스트(message 2)와 빠른 심볼 데이터(message 4)를 분리
        # → DeepSeek가 macro/events/fg/sc를 prefix로 캐시 → 히트율 향상
        context = build_unified_plan_context(macro, req.event_calendar, req.fear_greed, req.stablecoin)
        data = build_unified_plan_data(all_snapshots, recent_losses)
        async with deepseek_sem:
            text, ms, tokens = await generate_deepseek_plan(
                context, data, DEEPSEEK_UNIFIED_PLAN_SYSTEM, max_tok_deepseek, "unified_plan"
            )
