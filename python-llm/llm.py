...
async def _generate_api(url: str, model: str, api_key: str,
    ...
    messages 인자가 있으면 직접 사용 (4-message 구조 등), 없으면 system+prompt로 구성.
    """
    start = time.monotonic()
    collected = []
    reasoning_collected = []
...
async def generate_deepseek_plan(context: str, data: str, system: str,
                                 max_tokens: int, task_type: str = "unified_plan"):
    """DeepSeek 4-message 구조 호출 — prefix 캐시 히트율 극대화
    [system]    정적 규칙          → 항상 캐시됨
    [user]      느린 컨텍스트      → macro/events/fg/sc 안 바뀌면 캐시됨
    [assistant] 고정 응답          → 캐시됨
    [user]      빠른 심볼 데이터   → 매번 새 토큰
    """
    call_id = _start_call("deepseek", task_type, data)
    print(f"[LLM-Track] START {call_id} provider=deepseek task={task_type} ctx_len={len(context)} data_len={len(data)}")
    messages = [
        {"role": "system", "content": system},
        {"role": "user", "content": context},
        {"role": "assistant", "content": "Context loaded. Provide symbol data."},
        {"role": "user", "content": data},
    ]
    try:
        return await _generate_api(DEEPSEEK_URL, DEEPSEEK_MODEL, DEEPSEEK_API_KEY,
                                   "", system, max_tokens, call_id,
                                   timeout_read=120.0, messages=messages)
    finally:
        print(f"[LLM-Track] DONE  {call_id} output_len={len(_active_calls.get(call_id, {}).get('output', ''))}")
        _finish_call(call_id)
