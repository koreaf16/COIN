/**
 * @module 시장 상태 분석
 * @description 시간대별 성과 및 변동성 레짐/트렌드 강도에 따른 거래 성과를 분석한다.
 *
 * ┌──────────┐     ┌──────────┐     ┌──────────┐
 * │ Oracle   │ ──→ │ Market   │ ──→ │ Logger   │
 * │ DB       │     │ State    │     │ Output   │
 * └──────────┘     └──────────┘     └──────────┘
 *
...
async function main() {
    try {
        ...
        try {
            logger.info('--- 1. Hourly Performance Analysis (UTC) ---');
            const hourlyQuery = `
                SELECT 
        ...
        logger.error("Error in analyze_market_state:", e);
        process.exit(1);
    }
}
