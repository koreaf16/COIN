...
console.log(`\n${'='.repeat(60)}`);
console.log(`  COIN 포트폴리오 청산 + L2 이후 데이터 초기화`);
console.log(`  모드: ${testnet ? 'TESTNET' : '⚠️  MAINNET (실전)'}`);
...
if (apiKey) {
  try {
    ...
    if (openPositions.length === 0) {
      console.log('  → 열린 포지션 없음 ✓');
    } else {
      console.log(`  → ${openPositions.length}개 포지션 청산 시작...`);
      ...
      for (const pos of openPositions) {
        ...
        try {
          ...
          console.log(`  ✓ ${side} ${symbol} qty=${qty} 청산 완료 (orderId=${result.orderId})`);
        } catch (err) {
          console.error(`  ✗ ${symbol} 청산 실패: ${err.message}`);
        }
      }
    }
    ...
    if (openOrders.length > 0) {
      ...
      for (const sym of symbols) {
        try {
          ...
          console.log(`  ✓ ${sym} 주문 취소 완료`);
        } catch (err) {
          console.error(`  ✗ ${sym} 주문 취소 실패: ${err.message}`);
        }
      }
    } else {
      console.log('\n[2/3] 미체결 주문 없음 ✓');
    ...
    console.error(`  거래소 접근 실패: ${err.message}`);
    console.log('  → DB 초기화만 진행합니다.');
  }
} else {
  console.log('  → Binance API key 없음 — 거래소 작업 건너뜀');
  console.log('\n[2/3] 건너뜀');
}
...
console.log(`\n${'='.repeat(60)}`);
console.log('  완료! 포트폴리오 청산 + L2 이후 데이터 초기화됨');
console.log('  Z0 (Raw Data) / Z1 (Processed) 데이터는 유지됩니다.');
console.log(`${'='.repeat(60)}\n`);
