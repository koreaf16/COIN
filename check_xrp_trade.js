...
async function main() {
  ...
  try {
    const q = `
      ...
      FROM z4_positions
      WHERE symbol = 'XRPUSDT' AND direction = 'SHORT'
    ...
    if (r.rows.length > 0) {
      ...
      fs.writeFileSync('C:\\COIN\\tmp_xrp_trade.json', JSON.stringify(data, null, 2), 'utf8');
      console.log('Saved trade details to tmp_xrp_trade.json');
    } else {
