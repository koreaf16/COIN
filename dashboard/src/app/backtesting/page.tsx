            ...
            <tbody className="text-sm">
              {trades.length === 0 ? (
                ...
                <tr key={i} className="border-b border-surface-container-high/10 hover:bg-surface">
                  ...
                  <td className="p-3"><span className={`px-2 py-0.5 rounded text-[9px] font-bold ${t.exit_reason === 'TARGET' ? 'bg-success/10 text-success' : t.exit_reason === 'SAFETY_STOP' ? 'bg-error/10 text-error' : 'bg-surface-variant text-outline'}`}>{t.exit_reason || '--'}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
